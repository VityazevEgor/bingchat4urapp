...
## Веб-интерфейс
Вам также доступен веб-интерфейс по пути `127.0.0.1:8080/` который позволит вам проверить работу приложения
...
>**Важно:** авторизация в OpenAI(ChatGPT) и DeepSeek происходит используя Google аккаунт. Вы должны самостоятельно войти в свой Google аккаунт в окне браузера, которое открывается при запуске сервера, и только потом использовать этот endpoint для создания задачи на авторизацию в OpenAI или DeepSeek, передавая в качестве логина и пароля любые строки.
```http
POST /auth
```
